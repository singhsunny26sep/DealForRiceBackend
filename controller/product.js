  ...
  try {
    if (id) {
      ...
      if (product) {
        return res
          .status(200)
          .json({ success: true, msg: "Product found", result: product });
      }
    ...
    return res
      .status(200)
      .json({ success: true, msg: "All products found", result });
  } catch (error) {
...
exports.getLatestProducts = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, msg: "Latest products found", result });
  } catch (error) {
...
exports.getProductByUserId = async (req, res) => {
  ...
  try {
    ...
    const result = await Product.find({ user: id })
      .populate({ path: "user", select: "-password" })
      .populate("trade");
    if (result) {
      return res
        .status(200)
        .json({ success: true, msg: "Products found", result });
    }
    return res.status(404).json({ success: false, msg: "No products found" });
  } catch (error) {
...
exports.addProduct = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(400)
      .json({ success: false, msg: "Failed to add product!" });
  } catch (error) {
...
exports.getAdminProduct = async (req, res) => {
  try {
    ...
    const result = await Product.find({ user: userIds })
      .populate({
        path: "user",
        select: "name email mobile address city state image",
      })
      .populate("trade");
...
exports.updateProduct = async (req, res) => {
  ...
  try {
    ...
    if (result) {
      return res
        .status(200)
        .json({ success: true, msg: "Product updated successfully", result });
    }
    return res
      .status(400)
...
exports.deleteProduct = async (req, res) => {
  ...
  try {
    ...
    if (checkProdcut.image) {
      await deleteFromCloudinary(checkProdcut?.image);
    }
    const result = await Product.findByIdAndDelete(id);
    if (result) {
      return res.status(200).json({
        success: true,
        msg: `Product ${checkProdcut?.name} deleted successfully`,
      });
    }
    return res.status(404).json({ success: false, msg: "Product not found" });
  } catch (error) {
